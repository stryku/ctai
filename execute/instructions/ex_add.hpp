...
namespace ctai
{
    namespace execute2
    {
        ...
        struct ex_instruction<thread_t, machine_state_t, inst::to_size<inst::id_t::ADD_REG_VAL>, reg1, val, rest_of_opcodes...>
        {
            ...
            using add_result = details::ex_add_impl<thread_t, machine_state_t, reg1, val>;

            using result_thread = typename add_result::result_thread;
            using result_machine_state = typename add_result::result_machine_state;
        };

        template<typename thread_t,
                 typename machine_state_t, size_t reg1, size_t reg2, size_t ...rest_of_opcodes>
        struct ex_instruction<thread_t, machine_state_t, inst::to_size<inst::id_t::ADD_REG_REG>, reg1, reg2, rest_of_opcodes...>
        {
            using registers = typename thread_t::registers;

            using add_result = details::ex_add_impl<thread_t, machine_state_t, reg1, get_reg<registers, regs::to_id<reg2>>>;
