...
namespace ctai
{
    namespace execute2
    {
        ...
        struct ex_instruction<thread_t, machine_state_t, inst::to_size<inst::id_t::JL>, ip, rest_of_opcodes...>
        {
            static constexpr auto flags_v = typename thread_t::flags{};

            static constexpr bool should_jmp = (flags_v.CF == true);
            using final_regs_state = std::conditional_t<
                    should_jmp,
                    set_reg<typename thread_t::registers, regs::id_t::EIP, static_cast<uint32_t>(ip)>,
                    adjust_eip<typename thread_t::registers, inst::id_t::JL>>;
